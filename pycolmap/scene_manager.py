...
class SceneManager:
        ...
        with open(input_file, "rb") as f:
            ...
            for _ in range(num_cameras):
                ...
                params = struct.unpack(
                    "d" * num_params, f.read(8 * num_params)
                )
    ...
    def _load_images_bin(self, input_file):
        ...
        with open(input_file, "rb") as f:
            ...
            for _ in range(num_images):
                ...
                camera_id = data[8]
                name = b"".join(
                    c for c in iter(lambda: f.read(1), b"\x00")
                ).decode()
                ...
                points_elements = np.array(points_array).reshape(
                    (num_points2D, 3)
                )
                ...
                image.point3D_ids = np.array(
                    ids_array, dtype=np.uint64
                ).reshape((num_points2D,))
            ...
            for line in iter(lambda: f.readline().strip(), ""):
                ...
                if is_camera_description_line:
                    ...
                    image.points2D = np.array(
                        [map(float, data[::3]), map(float, data[1::3])]
                    ).T
    ...
    def _save_points3D_bin(self, output_file):
        ...
        with open(output_file, "wb") as fid:
            ...
            for point3D_id, point3D_idx in iter_point3D_id_to_point3D_idx:
                ...
                fid.write(
                    struct.pack(
                        "L", len(self.point3D_id_to_images[point3D_id])
                    )
                )
        ...
        with open(output_file, "w") as fid:
            ...
            for point3D_id, point3D_idx in iter_point3D_id_to_point3D_idx:
                ...
                print >> fid, array_to_string(
                    self.point3D_colors[point3D_idx]
                ),
    ...
    def get_points3D(
        self, image_id, return_points2D=True, return_colors=False
    ):
    ...
    def filter_points3D(
        ...
        for point3D_id, point3D_idx in iter_point3D_id_to_point3D_idx:
            ...
            # find dot product between all camera viewing rays
                ...
                if (
                    np.min(cos_theta) > max_tri_prod
                    or np.max(cos_theta) < min_tri_prod
                ):
                    self.point3D_id_to_point3D_idx[
                        point3D_id
                    ] = SceneManager.INVALID_POINT3D
