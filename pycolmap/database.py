...
class COLMAPDatabase(sqlite3.Connection):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.initialize_cameras = lambda: self.executescript(
            CREATE_CAMERAS_TABLE
        )
        self.initialize_descriptors = lambda: self.executescript(
            CREATE_DESCRIPTORS_TABLE
        )
        self.initialize_images = lambda: self.executescript(
            CREATE_IMAGES_TABLE
        )
        self.initialize_inlier_matches = lambda: self.executescript(
            CREATE_INLIER_MATCHES_TABLE
        )
        self.initialize_keypoints = lambda: self.executescript(
            CREATE_KEYPOINTS_TABLE
        )
        self.initialize_matches = lambda: self.executescript(
            CREATE_MATCHES_TABLE
        )
